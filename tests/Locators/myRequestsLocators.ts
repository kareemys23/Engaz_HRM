...
export class MyRequestsLocators {
    ...
    constructor(page: Page) {
        ...
        this.createRequestButton = page.getByRole('button', { name: 'Create New Request' });
        ...
        this.requestDeliveryDate = page.locator('.ng-untouched > .custom-date-picker > .date-picker-input-wrapper > #date-picker-undefined > .date-picker-icon > svg');
        this.deliveryDatePicker = page.locator('div').filter({ hasText: /^28$/ }).nth(1);
        this.requestNotes = page.getByRole('textbox', { name: 'Notes *' });
        this.requestAttachments = page.getByRole('img', { name: 'upload' });
        this.continueRequestButton = page.getByRole('button', { name: 'Continue' });
        this.submitRequestButton = page.getByRole('button', { name: 'Confirm Submit' });
